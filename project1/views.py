...
from project1.models import Department, UserInfoForm
...
def department_search(request):
    ...
    departments = Department.objects.filter(title__icontains=keyword)
    return render(request, "depart_list.html", {"departments": departments})
...
def user_list1(request):
    ...
    queryset = UserInfoForm.objects.all()
    """
    # logger.debug("Queryset: %s", queryset)
    # for obj in queryset:
    #     logger.debug(
    #         "User ID: %s, Name: %s, Account: %s", obj.id, obj.name, obj.account
    #     )
    #     logger.debug(
    #         "Create Time: %s, Gender: %s, Department ID: %s, Department Title: %s",
    ...
    # for obj in queryset:    !!! obj.get_gender_display()只要是可选择的都可以 这么写  obj.depart可以直接找到关联的表
...
def user_add1(request):
    ...
    if request.method == "GET":
        context = {
            "gender_choices": UserInfoForm.gender_choices,#男女
            "depart_list": Department.objects.all(),#部门
        }
...
def user_model_form_add(request):
    ...
    if form.is_valid():
        ...
        # models.UserInfoForm.objects.create(..)
        form.save()
        return redirect("/user/list/")
